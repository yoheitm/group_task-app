...
組織の中で複数のチームに分かれて作業を行うことは効率的ですが、スケジュール内容を変更する時や新たにイベントを追加する時などの意思決定と確認を瞬時にチーム内のメンバーで共有することが出来ると便利だと考えました。
...
## ユーザー管理機能
ユーザーはアカウントにアイコンを登録することができます。<br>ユーザー同士の距離が密接なアプリケーションになっているので、この機能によってユーザー同士がコミュニケーションがし易くなると思います。

## グループ管理機能
ユーザーはグループに登録している全てのユーザーを呼ぶことができます。<br>また、作成したグループからは参加・退室が任意で行えるので、他のグループに移動することとなった場合や間違えてメンバーを選んでしまった場合なども柔軟に対応できます。
