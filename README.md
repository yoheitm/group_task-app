...
組織の中で複数のチームに分かれて作業を行うことは多々ありますが、スケジュール内容を変更する時や新たにイベントを追加する時などの意思決定と確認を瞬時にチーム内のメンバーで共有することが出来ると便利だと考えました。
...
現状、イベントが始まる日時は登録できますが終了日時を登録することができていません。開始したイベントがいつ終了するのかが分からないと次の計画も立て辛いと考えます。今後、終了日時も登録できるようにしてカレンダー上にイベントを期間でライン表示できるようにしたいと考えています。
